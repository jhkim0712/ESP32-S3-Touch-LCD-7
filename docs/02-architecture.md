...
**제약:** 기기 JPEG 디코더는 progressive JPEG 를 풀지 못합니다. 받은 파일의 헤더(SOF 마커)가 progressive 이면
같은 이미지를 `wsrv.nl` 이미지 프록시로 다시 받아(800x800 안쪽으로 축소한 baseline JPEG) 바꿔 둡니다.
Instagram(rss.app) 처럼 서명된 URL 이라 크기/형식을 바꿔 요청할 수 없는 CDN 때문입니다.
프록시가 실패하면 원본을 남겨 두고(웹 UI 의 "표시할 수 없는 사진" 수에 포함) 다음 동기화 때 다시 시도합니다. WebP/GIF/AVIF 는 지원하지 않습니다.
