// Flickr 사진 피드 API (app_flickr)
//   GET  /api/flickr         {feeds:[url...], status:{syncing, last_sync, image_count, unsupported, error}}
//   POST /api/flickr         {feeds:[url...]} → NVS 저장 + 바로 동기화 (삭제한 피드의 사진도 지워짐)
//   POST /api/flickr/sync    지금 동기화
// 동기화 자체는 net_worker 가 한다 (APP_EVT_REQ_FLICKR_SYNC).
...
static const char *TAG = "web_flickr";

static bool valid_feed_url(const char *url)
{
    size_t len = strlen(url);
    if (len >= SETTINGS_FLICKR_URL_MAX || (strncmp(url, "https://", 8) != 0 && strncmp(url, "http://", 7) != 0)) {
        return false;
    }
...
static esp_err_t get_flickr(httpd_req_t *req)
{
    if (!web_authorized(req)) {
        return ESP_OK;
    }
    char *feeds = heap_caps_malloc(SETTINGS_FLICKR_FEEDS_LEN, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (!feeds) {
        return web_send_error(req, "500 Internal Server Error", "no memory");
    }
    settings_get_flickr_feeds(feeds, SETTINGS_FLICKR_FEEDS_LEN);
    ...
    cJSON_AddNumberToObject(root, "max_feeds", SETTINGS_FLICKR_MAX_FEEDS);
    return web_send_json(req, root);
}

static esp_err_t post_flickr(httpd_req_t *req)
{
    if (!web_authorized(req)) {
        return ESP_OK;
    }
    ...
    if (!cJSON_IsArray(list) || cJSON_GetArraySize(list) > SETTINGS_FLICKR_MAX_FEEDS) {
        cJSON_Delete(root);
        return web_send_error(req, "400 Bad Request", "feeds");
    }
    char *feeds = heap_caps_calloc(1, SETTINGS_FLICKR_FEEDS_LEN, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (!feeds) {
        cJSON_Delete(root);
        return web_send_error(req, "500 Internal Server Error", "no memory");
    }
    ...
    cJSON_ArrayForEach(item, list) {
        ...
        if (feeds[0]) {
            strlcat(feeds, "\n", SETTINGS_FLICKR_FEEDS_LEN);
        }
        strlcat(feeds, url, SETTINGS_FLICKR_FEEDS_LEN);
    }
    cJSON_Delete(root);

    esp_err_t err = bad ? ESP_ERR_INVALID_ARG : settings_set_flickr_feeds(feeds);
    free(feeds);
    if (bad) {
    ...
    app_event_post(APP_EVT_REQ_FLICKR_SYNC, NULL, 0);
...
esp_err_t web_flickr_register(httpd_handle_t server)
{
    static const httpd_uri_t uris[] = {
        { .uri = "/api/flickr",      .method = HTTP_GET,  .handler = get_flickr },
        { .uri = "/api/flickr",      .method = HTTP_POST, .handler = post_flickr },
        { .uri = "/api/flickr/sync", .method = HTTP_POST, .handler = post_sync },
    };
