...
//
// - 프레임버퍼(각 750KB)는 PSRAM. 2개면 LVGL 이 직접 그린다 (tearing 방지), 1개면 회전 복사 모드
...
esp_err_t board_lcd_init(uint8_t num_fbs, esp_lcd_panel_handle_t *out_panel)
{
    ...
    ESP_LOGI(TAG, "RGB panel %dx%d @ %d MHz, %d PSRAM FB(s), bounce %d lines",
             BOARD_LCD_H_RES, BOARD_LCD_V_RES, BOARD_LCD_PCLK_HZ / 1000000, num_fbs, BOARD_LCD_BOUNCE_LINES);
    *out_panel = panel;
    return ESP_OK;
}
